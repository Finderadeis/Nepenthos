...
public class PlayerController : MonoBehaviour, IDamageable, ISaveable
{
    ...
    private void ChangeHeightUpdate()
    {
        float stepRangeHeight = (2f - crouchHeight) * Time.deltaTime * 4;
        float stepRangeWidth = (0.5f - (crouchHeight/2f)) * Time.deltaTime * 4;

        if (currentWidth > targetWidth)
        {
            currentWidth -= stepRangeWidth;
            if (currentWidth < targetWidth)
            {
